  ...
  fetch(`https://api.github.com/users/${user_name}`)
    ...
    .then(function (data) {
      ...
      if (data.message === "Not Found") {
        ...
        user_img.onclick = () => {
          window.open(`https://github.com/${user_name}`, "_blank").focus();
        };
        userName.onclick = () => {
          window.open(`https://github.com/${user_name}`, "_blank").focus();
        };
      ...
      if (repo_data.length <= 0) {
        ...
        if (repo_data.message === "Not Found") {
          ...
          let repo_Data = repo_data.map((item) => {
            ...
            return `
                            <div class="item_">
                                <div class="repo_name" onclick="window.open('${item.html_url}', '_blank').focus()" style="cursor: pointer;" onMouseOver="this.style.color='#fff'"
                                onMouseOut="this.style.color='#10dd81'">${item.name} <<---</div>
                                <div class="repo_details_">
                                    <div class="info_ star">
                                        <i class="fa fa-star-o"></i>
                                        ${item.watchers}
